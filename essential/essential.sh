    ...
    while true;do
        ...
        if [[ $mode == cli ]];then
            ...
            *)
            ...
            3)  raw_pkgs=$(dialog --backtitle "corechunk : linutils --> [ https://github.com/corechunk/linutils.git ]" --title "essentials_desktop" --checklist "Select/toggle preffered options : " "$rows" "$cols" "$scroll" "${essentials_desktop_dialog[@]}" 2>&1 >/dev/tty); read -ra raw_pkgs <<< "$raw_pkgs"; pkgs=(); for ((i=0;i<${#raw_pkgs[@]};i++)); do [[ ${raw_pkgs[$i]} == *#* ]] && continue; pkgs+=("${pkgs[$i]}"); done; prompt_install_type "${pkgs[@]}" ;;
            4)  raw_pkgs=$(dialog --backtitle "corechunk : linutils --> [ https://github.com/corechunk/linutils.git ]" --title "essentials_hyprland" --checklist "Select/toggle preffered options : " "$rows" "$cols" "$scroll" "${essentials_hyprland_dialog[@]}" 2>&1 >/dev/tty); read -ra raw_pkgs <<< "$raw_pkgs"; pkgs=(); for ((i=0;i<${#raw_pkgs[@]};i++)); do [[ ${raw_pkgs[$i]} == *#* ]] && continue; pkgs+=("${pkgs[$i]}"); done; prompt_install_type "${pkgs[@]}" ;;
            5)  raw_pkgs=$(dialog --backtitle "corechunk : linutils --> [ https://github.com/corechunk/linutils.git ]" --title "corechunk's_hyprland_pkgs" --checklist "Select/toggle preffered options : " "$rows" "$cols" "$scroll" "${corechunk_hyprland_dialog[@]}" 2>&1 >/dev/tty); read -ra raw_pkgs <<< "$raw_pkgs"; pkgs=(); for ((i=0;i<${#raw_pkgs[@]};i++)); do [[ ${raw_pkgs[$i]} == *#* ]] && continue; pkgs+=("${pkgs[$i]}"); done; prompt_install_type "${pkgs[@]}" ;;
            5_force)
                for pkg in "${corechunk_hyprland[@]}"; do install_pkg_dynamic "$pkg" install-force; done ;;
            6) clear; menu_github_pkgs ;;
            9) menu_info ;;
